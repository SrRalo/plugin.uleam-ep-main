...
- Almacenamiento estructurado en tablas propias: `plugin_uleam_templates`, `plugin_uleam_forms`, `plugin_uleam_respuestas`.
- Validacion y sanitizacion de datos en envio publico.
- Soporte para tipos de campo especiales: fecha (formatos configurables), link seguro, firma digital por canvas (Base64), archivos.
...
2. Verifica que el directorio `vendor/` este incluido (dependencias de Dompdf).
3. Activa el plugin desde el menu Plugins en WordPress.
4. En el admin, abre `ULEAM Formularios` y crea un formulario desde `Constructor`.
5. Publica el formulario con el shortcode `[uleam_form id="ID_DEL_FORMULARIO"]`.
...
2. Vista del Constructor con secciones, campos y configuraciones.
3. Vista publica del formulario renderizado por shortcode.
4. Modulo de Analiticas con filtros y exportacion CSV/PDF.
